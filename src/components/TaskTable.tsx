                ...
                {isExpanded && (
                  <div className="border-t border-gray-50 bg-gray-50/30 px-4 py-6 animate-in fade-in slide-in-from-top-2 duration-200">
                    <div className="flex flex-col gap-6 relative">
                      ...
                      {(['firstDraft', 'secondDraft', 'thirdDraft', 'fourthDraft', 'publish'] as const).map((key, i) => {
                        const milestone = task.milestones?.[key];
                        const label = key === 'publish' ? '完了' : key === 'firstDraft' ? '初稿' : `${key.replace('Draft', '').replace('second', '2').replace('third', '3').replace('fourth', '4')}校`;
                        const stepNum = i + 1;
                        const completedCount = getCompletedSteps(task.status);
                        const isCompleted = stepNum <= completedCount;
                        const isCurrent = stepNum === getCurrentWorkingStep(task.status);
                          ...
                          <div key={key} className="flex items-start gap-4 relative z-10">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex justify-between items-center mb-1">
                                ...
                                <span className="text-[10px] font-mono text-gray-400">{formatDate(milestone?.deadline)}</span>
                              </div>
                              {milestone?.url ? (
                                <div className="flex gap-2 mt-1">
                                  <a href={milestone.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[10px] text-white bg-blue-600 px-3 py-1 rounded shadow-sm">
                                    <LinkIcon size={12} />開く
                                  </a>
                                  <button 
                                    onClick={() => handleCopy(milestone.url!, `${task.id}-${key}`)}
                                    className={`flex items-center gap-1 text-[10px] px-3 py-1 rounded border transition-all ${copiedId === `${task.id}-${key}` ? 'bg-green-50 text-green-600 border-green-200' : 'bg-white text-gray-600 border-gray-200'}`}
                                  >
                                    {copiedId === `${task.id}-${key}` ? <Check size={12} /> : <Copy size={12} />}
                                    {copiedId === `${task.id}-${key}` ? '完了' : 'コピー'}
                                  </button>
                                </div>
                              ) : (
                                <span className="text-[10px] text-gray-300">URL未設定</span>
